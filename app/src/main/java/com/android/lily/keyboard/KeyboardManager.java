...
public class KeyboardManager {
    ...
    public KeyboardManager(Context context) {
        ...
        if (mContext instanceof Activity) {
            ...
            mKeyboardWithSearchView = (KeyboardWithSearchView) LayoutInflater.from(context).inflate(R.layout.layout_keyboard_view, null);
            hideSystemSoftKeyboard(mKeyboardWithSearchView.getEditText());
            mKeyboardContainerLayoutParams = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
...
//        mRootView.getViewTreeObserver().addOnGlobalLayoutListener(mOnGlobalLayoutListener);
        ...
        mKeyboardWithSearchView.getKeyboardViewContainer().setPadding(
                Utils.dipToPx(mContext, keyboard.getPadding().left),
...
//            }
            ...
            if (mKeyboardWithSearchView.getVisibility() == View.GONE) {
                ...
                int keyboardTop = etLocation[1] + editText.getHeight() + editText.getPaddingTop() + editText.getPaddingBottom() + 1; // 1px is divider
                ...
                if (mShowAnchorView != null) {
                    ...
                    keyboardTop = saLocation[1] + mShowAnchorView.getHeight() + mShowAnchorView.getPaddingTop() + mShowAnchorView.getPaddingBottom() + 1; // 1px is divider
                }
                int moveHeight = keyboardTop + mKeyboardWithSearchView.getHeight() - rect.bottom;
                // height > 0 root view 需要继续上滑
                if (moveHeight > 0) {
